...
import { getWorkingHours, WorkingHours } from "./utils";
...
const splitAvailableTime = (
  ...
  while (initialTime < finalizationTime) {
    ...
    */
    if (slotEndTime <= finalizationTime + 1)
...
const getSlots = ({
  ...
  const makeTimeFrame = (
    item: typeof localWorkingHours[0]
  ...
  const uniq = (a: Dayjs[]) => {
    ...
    return a.filter((item) => {
      return seen.hasOwnProperty(item.format())
        ? false
        : (seen[item.format()] = true);
    });
  };
