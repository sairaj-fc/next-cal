...
import { daysInMonth, yyyymmdd } from "@lib/utils";
...
const SkeletonText: React.FC<SkeletonBaseProps> = ({
  width,
  height,
  className,
}) => {
  return (
    <div
      className={classNames(
        `animate-pulse rounded-md bg-gray-300 w-${width} h-${height}`,
        className
      )}
    />
  );
...
export const Day = ({
  ...
  return (
    ...
    >
      {date.date()}
      {date.isToday() && (
        <span className="absolute left-0 bottom-0 mx-auto -mb-px w-full text-4xl">
          .
        </span>
      )}
    </button>
      ...
      {days.map((day, idx) => (
        ...
        >
          {day === null ? (
            ...
            <button
              className="absolute top-0 left-0 right-0 bottom-0 mx-auto flex w-full items-center justify-center rounded-sm border-transparent bg-gray-50 text-center text-gray-400 opacity-50 "
              key={`e-${idx}`}
              disabled
            >
    ...
    <div className={className}>
      <div className="mb-4 flex justify-between text-xl font-light">
        <span className="w-1/2 ">
          {browsingDate ? (
            <>
              <strong className="text-bookingdarker ">
        ...
        <div className="text-black ">
          <button
            ...
            className={classNames(
              "group p-1 opacity-50 hover:opacity-100 ltr:mr-2 rtl:ml-2 text-gray-900",
          ...
          <button
            className="group p-1 opacity-50 hover:opacity-100 text-gray-900"
            onClick={() => changeMonth(+1)}
      ...
      <div className="border-bookinglightest mb-2 grid grid-cols-7 gap-4 border-t border-b text-center md:mb-0 md:border-0">
